...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(p("Try manipulating the values of predictors below and observe how the changes
              ...
              "as important variables and they were placed in the left-hand panel"),
              br(),
              br()
              )
  ),
  ...
  fluidRow(
    column(4, 
           "features set 1",
           numericInput("Glucose", label = "Glucose", value = median(data$Glucose), min = 1,max=199),
           numericInput("BMI", label = "BMI", value = median(data$BMI),min=0,max=67.1),
           numericInput("Age", label = "Age", value = median(data$Age), min = 21, max=81, step=1)
    ),
    column(4, 
           "features set 2",
           numericInput("Pregnancies", label = "Pregnancies", value = median(data$Pregnancies), min = 0, max=17, step = 1),
           numericInput("BloodPressure", label = "Blood Pressure", value = median(data$BloodPressure), min=0, max=122, step = 1),
           numericInput("DiabetesPedigree", label = "Diabetes Pedigree Function", value = median(data$DiabetesPedigreeFunction), 
                        min = 0.078, max= 2.42)
    ),
    column(4,
           "features set 3",
           numericInput("Insulin", label = "Insulin", value = median(data$Insulin), min=0, max=846),
           numericInput("SkinThickness", label = "Skin Thickness", value = median(data$SkinThickness), min=0, max=99)
    )
  ),
  fluidRow(
